...
class Curl implements \SAI\CurlInterface {
   ...
   */
  private $_optionss = array();

  public function close($ch) {
    unset($this->_optionss[$ch]);
  }

  public function copy_handle($ch) {
    $newCh = ++$this->_lastHandle;

    $handle = clone $this->_optionss[$ch];

    $this->_optionss[$newCh] = $handle;

    return $newCh;
  }

  public function errno($ch) {
    if(!isset($this->_optionss[$ch]))
      return 0;

    return $this->_getErrorCode($this->_optionss[$ch]);
  }
  ...
  public function exec($ch) {
    ...
    if($handle[CURLOPT_RETURNTRANSFER])
      return $response;
  ...
  public function getinfo($ch, $opt = 0) {
    ...
    if($opt == 0)
      return $this->_getInfoArray($this->_optionss[$ch]);

    return $this->_getInfo($this->_optionss[$ch], $opt);
  }
  ...
  public function setopt($ch, $option, $value) {
    ...
    $this->_optionss[$ch][$option] = $value;
